...
#'distance_m','time_diff_s','time_diff_s',
"""
for unit tests
condition = (df['segment'] != df['prev_segment']) & df['speed_kmph'].notna()

# Check if condition ever fails
if condition.any():
    print("❌ There are rows where A != B but C is not null.")
    print(df[condition])
else:
    print("✅ All rows satisfy: if A != B, then C is null.")
"""